...
The Dockerfile only installs the C++ deps (`pagexml`, `textfeat`); install the Python package once inside the container so the `htr-cli` CLI is available:

```bash
uv pip install --system -e . #--system skips the venv warning
```

The TFE-based preprocessing command is `htr-cli process-images-tfe` (see `src/htr_cli/image_processing_tfe.py`).
